...
export const ActionsForPeople = {
    gatherPpl: function () {
        ...
        axios.get('https://localhost:44322/api/People/GetAll?') //http://tc-dmv.azurewebsites.net
            .then(res => {
                Dispatcher.dispatch({
...
export const ActionsForSearcher = {
    finder: function (name) {
        ...
        axios.get('https://localhost:44322/api/People/Find?',{//http://tc-dmv.azurewebsites.net
            params:{
                FN:name.firstname,
                LN:name.lastname
            }
...
export const ActionsForCreator = {
    create: function (person) {
        ...
        axios.post(`https://localhost:44322/api/People/Create?newFN=${person.firstname}&newLN=${person.lastname}&newBDAY=${person.bday}&newSSN=${person.ssn}&newEMAIL=${person.email}&newSTREET1=${person.street1}&newSTREET2=${person.street2}&newCITY=${person.city}&newSTATE=${person.state}&newZIP=${person.zip}`)
        .then(res => {
            Dispatcher.dispatch({
